...
const specifiedWidth =
  (Dimensions.get('window').width -
    space[1].substr(0, 1) * 8 -
    space[2].substr(0, 2) * 2) /
  4;
...
class CheckoutScreen extends BaseScreen {
  constructor(props) {
    ...
    this.state = {
      ...
      name:
        this.props.UserStore.userName != null
          ? this.props.UserStore.userName +
            ' ' +
            this.props.UserStore.userSurname
          : '',
      phone:
        this.props.UserStore.userPhone != null
          ? this.props.UserStore.userPhone
          : '',
  ...
  /////MODALS
  hideSelectListModal = () => {
    this.setState({selectListModalVisible: false});
  };
  showSelectListModal = () => {
    this.setState({selectListModalVisible: true});
  };
  onSelect = item => {
    this.setState(
      {
        selectedPaymentMethodValue: item.value,
        selectedPaymentMethodName: item.name,
      },
      () => {
  ...
  jumpHome = () => {
    this.props.navigation.jumpTo('homeNavigator');
  };
  goToUserTab = () => {
  ...
  goToPayment = async () => {
    if (this.state.name == '' || this.state.phone == '') {
      this.showErrorModal(
        I18n.t('$UyarilarSiparisBilgileriniEksiksizDoldurunuz'),
      );
