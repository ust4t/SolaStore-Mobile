...
class ProductDetail extends BaseScreen {
  constructor(props) {
    ...
    this.state = {
      ...
      isFavorite: this.props.UserStore.favorites.find(
        a => a.productID == this.props.route.params.productId,
      )
        ? true
        : false,
  ...
  ///////NAVIGATIONS
  goToUserTab = () => {
    this.props.navigation.jumpTo('userNavigator');
  };
  goToVideoPlayer = () => {
    this.props.navigation.navigate('ProductVideoPlayer', {
  ...
  getProductDetail = async () => {
    if (this.props.route.params.productId) {
      let data = await this.doRequestAsync(() =>
        productService.GetProductById(this.props.route.params.productId),
      );
  ...
  getVariations = async originalData => {
    if (this.props.route.params.productId) {
      let data = await this.doRequestAsync(() =>
        productService.GetVariationsByProductID(
  ...
  addToFavorites = async () => {
    if (this.state.isFavorite) {
      let resp = await this.doRequestAsync(() =>
        favoriteService.DeleteFavoriteProduct(this.item.productID),
      );
      if (resp.status == 200) {
      ...
      let resp = await this.doRequestAsync(() =>
        favoriteService.AddFavoriteProduct(this.item.productID),
      );
      if (resp.status == 200) {
        this.setState({isFavorite: !this.state.isFavorite});
  ...
  addToBasket = async () => {
    let rsp = await this.doRequestAsync(() =>
      basketService.addToBasket(this.item.productID, this.state.count),
    );
    if (rsp) {
  ...
  render() {
    return (
      <SafeArea style={{backgroundColor: color.lightGray}}>
        ...
        <ButtonWrapper>
          <PrimaryButton
            text={I18n.t('$AnaSayfasepeteekle')}
            action={this.addToBasket}
            paddingCount={2}
          />
          <SeperatorFromRightOrLeft />
          <SecondaryButton
            text={I18n.t('$AnaSayfaSepetGoster')}
            action={this.goToBasketTab}
            paddingCount={2}
          />
        </ButtonWrapper>
  ...
  shareProduct = async () => {
    await Share.share({
      title: this.item.productShortName,
