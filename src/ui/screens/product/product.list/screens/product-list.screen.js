...
const CalculatedImageSize = `${
  497 *
  ((deviceWidth / 2 -
    (parseInt(space[3].substring(0, 2)) +
      2 * parseInt(space[2].substring(0, 2)))) /
    331)
}`;
...
class ProductList extends BaseScreen {
  ...
  defaultRequestProcess = (products, filter = false) => {
    ...
    products.map(item => {
      if (
        this.props.UserStore.favorites.find(a => a.productID == item.productID)
      ) {
  ...
  getAllByCategoryID = async () => {
    if (this.props.route.params.categoryID) {
      let data = await this.doRequestAsync(() =>
        productService.GetAllByCategoryID(this.props.route.params.categoryID),
      );
      if (data) {
  ...
  getAllByBrandId = async () => {
    if (this.props.route.params.categoryID) {
      let data = await this.doRequestAsync(() =>
        productService.GetSelectedBrandProducts(
  ...
  getAllWithAdvancedSearch = async () => {
    ...
    let data = await this.doRequestAsync(() =>
      productService.advancedSearchProducts(
        categoryString,
        brandString,
        this.props.route.params.priceRange
          ? this.props.route.params.priceRange.substr(
              0,
              this.props.route.params.priceRange.indexOf('-'),
            )
          : 0,
        this.props.route.params.priceRange
          ? this.props.route.params.priceRange.substr(
              this.props.route.params.priceRange.indexOf('-') + 1,
              2,
            )
          : 999,
      ),
  ...
  search = async () => {
    let response = await this.doRequestAsync(() =>
      productService.searchProducts(this.props.route.params.text),
    );
    if (response) {
  ...
  addToFavorites = async item => {
    ...
    if (favoriteState) {
      let resp = await this.doRequestAsync(() =>
        favoriteService.DeleteFavoriteProduct(item.productID),
      );
      if (resp.status == 200) {
      ...
      let resp = await this.doRequestAsync(() =>
        favoriteService.AddFavoriteProduct(item.productID),
      );
      if (resp.status == 200) {
  ...
  /////////////////////////////
  ////////NAVIGATION
  goToProductDetail = (productId, secondaryId) => {
    this.props.navigation.navigate('ProductDetail', {productId, secondaryId});
  };
  goToBasket = () => {
  ...
  goToProductWithSearchValues = () => {
    if (this.state.searchText.length != 0)
      this.props.navigation.navigate('ProductListSearch', {
        type: 'variation',
        variationType: 5,
  ...
  getItemLayout = (data, index) => ({
    length: CalculatedSupWrapperHeight,
    offset: CalculatedSupWrapperHeight * index,
    index,
  });
  render() {
    return (
      <SafeArea>
        ...
        {this.props.route.params == undefined && (
          <Tabbar
            ...
            navigatorName={'newProductNavigator'}
          />
        )}
      </SafeArea>
    );
  ...
  createParameters = async () => {
    ...
    if (this.state.selectedSortOption != '-') {
      if (
        this.state.selectedSortOption == I18n.t('$UrunlerFiyatiEnYuksekOlan')
      ) {
        newList.sort(this.dynamicSort('price')).reverse();
      } else if (
        this.state.selectedSortOption == I18n.t('$UrunlerFiyatiEnDusukOlan')
      ) {
    ...
    newList = newList.filter(
      x => x.price > this.state.minPrice && x.price < this.state.maxPrice,
    );
